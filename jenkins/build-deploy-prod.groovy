...
node {
    ...
    try {
        stage ('Load Libraries') {
            ...
            gitHelper = load "${SHARED_DEVOPS_PATH}/jenkins/scm/git.groovy"
        }
        ...
        stage ('Notify Last Changes') {
            if (params.enableReleaseTagging) {
                String releaseTagFilter = releaseTagPrefix + "_*"
                String previousDeployCommitHash = gitHelper.getCommitHashByTag(releaseTagFilter)
                String currentDeployCommitHash = gitHelper.getCurrentCommitHash()

                if (previousDeployCommitHash != '') {
                    String pathLimit = 'evilangel.com/main/ shared/'
                    lastChangesDiffMsg = gitHelper.getDiffMessages(previousDeployCommitHash, currentDeployCommitHash, pathLimit)
                }

                if (lastChangesDiffMsg == '')
                    lastChangesDiffMsg = 'No Changes Found'

                String prettyDiffMsg = gitHelper.getPrettyDiffMessages(releaseTagFilter, currentDeployCommitHash, lastChangesDiffMsg)
                slackHelper.sendBuildStatus('LAST CHANGES', prettyDiffMsg)
            }
        }
        ...
        stage ('Tag Release') {
            if (params.enableReleaseTagging) {
                gitHelper.tag(releaseTagPrefix, lastChangesDiffMsg)
            }
        }
